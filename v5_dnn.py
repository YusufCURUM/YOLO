...
class yolov5():
    ...
    def detect(self, srcimg):
        ...
        for i in range(self.nl):
            ...
            # outs[i] = outs[i][0].reshape(bs, self.na, self.no, ny, nx).transpose(0, 1, 3, 4, 2)

            outs_copy = outs[i][0].copy()
            outs_copy = outs_copy.reshape(bs, self.na, self.no, ny, nx).transpose(0, 1, 3, 4, 2)
            outs = list(outs)
            outs[i] = tuple(outs_copy.tolist())
            outs = tuple(outs)

            #if self.grid[i].shape[2:4] != outs[i].shape[2:4]:
            self.grid[i] = self._make_grid(nx, ny)

            y = 1 / (1 + np.exp(-np.array(outs[i])))  # convert tuple back to ndarray
