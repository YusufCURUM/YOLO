...
v4tiny_Net.setPreferableBackend(cv2.dnn.DNN_BACKEND_OPENCV)
v4tiny_Net.setPreferableTarget(cv2.dnn.DNN_TARGET_CPU)
...
def v4_inference(frame):
    ...
    v4tiny_Net.setInput(cv2.dnn.blobFromImage(frame, 1 / 255.0, (416, 416), swapRB=True, crop=False))
...
if __name__ == '__main__':
    ...
    cap.set(2, 700)  # set video width
    cap.set(3, 500)  # set video height
    while True:
        # begin = time.time()
        ret, frame = cap.read()
        v4_inference(frame)
